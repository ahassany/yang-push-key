...
use yang_push_key::{derive_templates, normalize_subtree, produce_kafka_key};
...
fn pipeline_p1_to_p2_to_p3_full_roundtrip() {
    ...
    let expected_key = include_str!("../assets/testdata/expected/p1_to_p2_to_p3.json");

    let xpath = normalize_subtree(&ctx, filter_xml).expect("Phase 1 failed");
    let derivation = derive_templates(&ctx, &xpath).expect("Phase 2 failed");
    let data = parse_data(&ctx, data_xml);
    let result =
        produce_kafka_key(&derivation, &data, "router-nyc-01", "1042").expect("Phase 3 failed");

    assert_eq!(result.kafka_key, expected_key);
}
...
fn pipeline_p1_to_p2_to_p3_nested_with_content_match() {
    ...
    let data = parse_data(&ctx, data_xml);
    let result =
        produce_kafka_key(&derivation, &data, "switch-dc-12", "7500").expect("Phase 3 failed");
